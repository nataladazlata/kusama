...
update() {
	...
	if ! grep -q "Image is up to date for" <<< "$status"; then
		...
		docker run -dit --name kusama_node --restart always --network host -v $HOME/.kusama:/data -u $(id -u ${USER}):$(id -g ${USER}) parity/polkadot --base-path /data --chain kusama --validator --name "nlz" --state-pruning=1000 --port 30533 --ws-port 9954 --rpc-port 9953 --prometheus-port 9655 --in-peers 25 --out-peers 25 --telemetry-url 'wss://telemetry-backend.w3f.community/submit 1' --telemetry-url 'wss://telemetry.polkadot.io/submit/ 1'
	else
